...
static bool8 HandleLoadTMCaseGraphicsAndPalettes(void)
{
    ...
    case 0:
        ...
        DecompressAndCopyTileDataToVram(1, gTMCase_bg, 0, 0, 0);
        sTMCaseDynamicResources->seqId++;
        break;
    case 1:
        if (FreeTempTileDataBuffersIfPossible() != TRUE)
        {
            LZDecompressWram(gTMCase_Tilemap, sTilemapBuffer);
            sTMCaseDynamicResources->seqId++;
        }
        break;
    case 2:
    ...
    case 3:
        if (gSaveBlock2Ptr->playerGender == MALE)
            LoadCompressedPalette(gTMCase_BagColor_Male, 0, 0x80);
        else
            LoadCompressedPalette(gTMCase_BagColor_Female, 0, 0x80);
        sTMCaseDynamicResources->seqId++;
        break;
    case 4:
