...
#define SPRITE_PALETTE_TAG_FOG       0x1201
#define SPRITE_PALETTE_TAG_SANDSTORM 0x1204
#define SPRITE_PALETTE_TAG_WEATHER   0x1207

const struct SpritePalette sFogSpritePalette =       { gFogPalette,              SPRITE_PALETTE_TAG_FOG };
const struct SpritePalette sSandstormSpritePalette = { gSandstormWeatherPalette, SPRITE_PALETTE_TAG_SANDSTORM };
const struct SpritePalette sCloudsSpritePalette =    { gCloudsWeatherPalette,    SPRITE_PALETTE_TAG_WEATHER };
#endif
//------------------------------------------------------------------------------
...
#ifdef FEATURE_DYNAMICOVERWORLDPALETTES
    FreeSpriteTilesByTag(SPRITE_PALETTE_TAG_WEATHER);
#else
...
static void UpdateRainSprite(struct Sprite *sprite)
{
    if (sprite->tState == 0)
    {
        ...
        if (sprite->tActive && (sprite->pos1.x >= -8 && sprite->pos1.x <= 248) && sprite->pos1.y >= -16 && sprite->pos1.y <= 176)
...
{
    // fixes bug where snow stops spawing
#ifdef BUGFIX
...
static const struct SpriteTemplate sAshSpriteTemplate =
    {
        .tileTag = GFXTAG_ASH,
#ifdef FEATURE_DYNAMICOVERWORLDPALETTES
        .paletteTag = SPRITE_PALETTE_TAG_FOG,
#else
        .paletteTag = PALTAG_WEATHER,
#endif
...
#ifdef FEATURE_DYNAMICOVERWORLDPALETTES
        .paletteTag = SPRITE_PALETTE_TAG_SANDSTORM,
#else
        .paletteTag = PALTAG_WEATHER_2,
#endif
